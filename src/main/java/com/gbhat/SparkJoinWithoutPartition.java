...
public class SparkJoinWithoutPartition {
    public static void main(String[] args) throws IOException {
        ...
        cityDs = cityDs.repartition(4).persist(StorageLevel.MEMORY_AND_DISK());;

        Dataset<Row> countryDs = session.read().parquet("country.parquet");

        countryDs = countryDs.repartition(4).persist(StorageLevel.MEMORY_AND_DISK());;
